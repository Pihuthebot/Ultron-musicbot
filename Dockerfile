...
RUN wget -q https://github.com/mrnitric/Ultron-musicbot/archive/dev.tar.gz && \
    tar xf dev.tar.gz && rm dev.tar.gz

WORKDIR /Ultron-musicbot
CMD python3 main.py

# docker build -t tgcalls .
# docker run -it --rm --env-file ./envfile --name Ultron-musicbot tgcalls
