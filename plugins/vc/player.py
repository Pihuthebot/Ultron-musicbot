...
USERBOT_HELP = f"""{emoji.LABEL}  **Uɭtʀoŋ Coɱɱoŋ Coɱɱʌŋɗs**:
...
class MusicPlayer(object):
    ...
    async def send_playlist(self):
        ...
        if not playlist:
            pl = f"{emoji.NO_ENTRY} Pɭʌƴɭɩst Is Eɱptƴ"
        else:
            if len(playlist) == 1:
...
async def network_status_changed_handler(gc: GroupCall, is_connected: bool):
    if is_connected:
        ...
        await send_text(f"{emoji.CROSS_MARK_BUTTON} Sʋccɘsʆʋɭɭƴ ɭɘʆt tʜɘ Voɩcɘ Cʜʌt")
        mp.chat_id = None
...
async def play_track(client, m: Message):
    ...
    if playlist and playlist[-1].audio.file_unique_id \
        ...
        reply = await m.reply_text(f"{emoji.ROBOT} Aɭʀɘʌɗƴ ʌɗɗɘɗ")
    ...
    if len(playlist) == 1:
        m_status = await m.reply_text(
            f"{emoji.INBOX_TRAY} Dowŋɭoʌɗɩŋʛ & Tʀʌŋscoɗɩŋʛ..."
        )
...
async def skip_track(client, m: Message):
    ...
    if len(m.command) == 1:
        ...
        try:
            ...
            reply = await m.reply_text(f"{emoji.NO_ENTRY} Iŋvʌɭɩɗ Iŋpʋt",
...
async def list_voice_chat(client, m: Message):
    ...
    if group_call.is_connected:
        ...
        reply = await m.reply_text(
            f"{emoji.MUSICAL_NOTES} **Cʋʀʀɘŋtɭƴ ɩŋ tʜɘ voɩcɘ cʜʌt**:\n"
            f"- **{chat.title}**"
        )
    else:
        reply = await m.reply_text(emoji.NO_ENTRY
                                   + "Dɩɗŋ't joɩŋ ʌŋƴ voɩcɘ cʜʌt ƴɘt")
    await _delay_delete_messages((reply, m), DELETE_DELAY)
...
async def pause_playing(_, m: Message):
    ...
    await mp.update_start_time(reset=True)
    reply = await m.reply_text(f"{emoji.PLAY_OR_PAUSE_BUTTON} Pɭʌƴɩŋʛ pʌʋsɘɗ",
                               quote=False)
    mp.msg['pause'] = reply
    await m.delete()
...
async def resume_playing(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.PLAY_OR_PAUSE_BUTTON} Pɭʌƴɩŋʛ ʀɘsʋɱɘɗ",
                               quote=False)
...
async def unmute(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.SPEAKER_MEDIUM_VOLUME} UnMʋtɘɗ")
    await _delay_delete_messages((reply, m), DELETE_DELAY)
