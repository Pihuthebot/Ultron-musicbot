...
USERBOT_HELP = f"""{emoji.LABEL}  **Uɭtʀoŋ Coɱɱʌŋ Coɱɱʌŋɗs**:
...
USERBOT_REPO = f"""{emoji.ROBOT} **Uɭtʀoŋ Voɩcɘ Cʜʌt Usɘʀɓot**

- Repository: [GitHub](https://github.com/mrnitric/Ultron-musicbot)
...
class MusicPlayer(object):
    ...
    async def send_playlist(self):
        ...
        if not playlist:
            pl = f"{emoji.NO_ENTRY} No Aŋƴ Pɭʌƴɭɩst Avʌɩɭʌɓɭɘ."
        else:
            if len(playlist) == 1:
                pl = f"{emoji.REPEAT_SINGLE_BUTTON} **Pɭʌƴɭɩst**:\n"
            else:
                pl = f"{emoji.PLAY_BUTTON} **Pɭʌƴɭɩst**:\n"
            pl += "\n".join([
...
async def network_status_changed_handler(gc: GroupCall, is_connected: bool):
    if is_connected:
        ...
        await send_text(f"{emoji.CHECK_MARK_BUTTON} Sʋccɘsʆʋɭɭƴ Joɩŋɘɗ tʜɘ voɩcɘ cʜʌt.")
    else:
        await send_text(f"{emoji.CROSS_MARK_BUTTON} Sʋccɘsʆʋɭɭƴ Lɘʌvɘɗ tʜɘ voɩcɘ cʜʌt.")
        mp.chat_id = None
...
async def play_track(client, m: Message):
    ...
    if playlist and playlist[-1].audio.file_unique_id \
        ...
        reply = await m.reply_text(f"{emoji.ROBOT} Aɭʀɘʌɗƴ Aɗɗɘɗ")
    ...
    if len(playlist) == 1:
        m_status = await m.reply_text(
            f"{emoji.INBOX_TRAY} Wʌɩt Mʌstɘʀ.. Aɱ tʀʌŋscoɗɩŋʛ Tʜɘ Soŋʛ."
        )
        ...
        print(f"- Stʌʀt pɭʌƴɩŋʛ : {playlist[0].audio.title}")
    await mp.send_playlist()
...
async def show_current_playing_time(client, m: Message):
    ...
    playlist = mp.playlist
    if not start_time:
        reply = await m.reply_text(f"{emoji.PLAY_BUTTON} Uŋĸŋowŋ")
        await _delay_delete_messages((reply, m), DELETE_DELAY)
        return
...
async def skip_track(client, m: Message):
    ...
    if len(m.command) == 1:
        ...
        try:
            ...
            reply = await m.reply_text(f"{emoji.NO_ENTRY} Iŋvʌɭɩɗ ɩŋpʋt",
...
async def list_voice_chat(client, m: Message):
    ...
    if group_call.is_connected:
        ...
        reply = await m.reply_text(
            f"{emoji.MUSICAL_NOTES} **Mƴ Mʌstɘʀ, Cʋʀʀɘŋtɭƴ ɩŋ tʜɘ voɩcɘ cʜʌt.**:\n"
            f"- **{chat.title}**"
        )
    else:
        reply = await m.reply_text(emoji.NO_ENTRY
                                   + "Mƴ Mʌstɘʀ, Dɩɗŋ't joɩŋ ʌŋƴ voɩcɘ cʜʌt ƴɘt")
    await _delay_delete_messages((reply, m), DELETE_DELAY)
...
async def stop_playing(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.STOP_BUTTON} Stoppɘɗ pɭʌƴɩŋʛ")
...
async def pause_playing(_, m: Message):
    ...
    await mp.update_start_time(reset=True)
    reply = await m.reply_text(f"{emoji.PLAY_OR_PAUSE_BUTTON} Soŋʛ pʌʋsɘɗ",
                               quote=False)
    mp.msg['pause'] = reply
    await m.delete()
...
async def resume_playing(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.PLAY_OR_PAUSE_BUTTON} Soŋʛ ʀɘsʋɱɘɗ",
                               quote=False)
...
async def clean_raw_pcm(client, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.WASTEBASKET} Cɭɘʌŋɘɗ {count} Fɩɭɘs")
    await _delay_delete_messages((reply, m), DELETE_DELAY)
...
async def mute(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.MUTED_SPEAKER} Mʋtɘɗ")
    await _delay_delete_messages((reply, m), DELETE_DELAY)
...
async def unmute(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.SPEAKER_MEDIUM_VOLUME} UŋMʋtɘɗ")
    await _delay_delete_messages((reply, m), DELETE_DELAY)
